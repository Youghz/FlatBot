...
6. BotFather vous donnera un **token** qui ressemble a ceci : `REDACTED_BOT_TOKEN`. Notez-le precieusement.
...
5. Cherchez le `chat_id` du groupe. **Attention** : les identifiants de groupe sont des nombres **negatifs** (par exemple : `REDACTED_CHAT_ID`). C'est normal.
